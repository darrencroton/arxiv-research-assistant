...
- `src/re_ass/arxiv_rate_limit.py`: shared crawl-delay pacing and User-Agent for the listing, abstract-page, and PDF requests in `arxiv_fetcher.py`/`generation_service.py`, plus the retry schedules the listing fetch and PDF download each use on top of that pacing (see AGENTS.md's Working Notes for exactly which call sites retry and which don't; the separate `arxiv.Client` metadata path has its own delay/retry and does not use this module); also holds `DEFAULT_HEADERS` (browser-like `Accept`/`Accept-Language`/`Accept-Encoding`, used by the listing and abstract-page fetches), `decode_html_response` (transparently un-gzips/inflates what those headers now invite), and `describe_http_error` (best-effort Retry-After/body extraction for diagnosing a transient status instead of logging a bare code)
...
- The listing fetch is the one call site still exposed to arxiv.org's main-site bot mitigation. The 2026-09-22 investigation assumed a 406 there clears within roughly an hour, so `_fetch_listing_html` was given a longer, dedicated `LISTING_RETRY_DELAYS_SECONDS` schedule (up to ~17 minutes across a run). That assumption did not hold: occurrences on 2026-09-22 and 2026-09-24 showed the block still active 4+ hours later, so the extra wait was retrying into a block that wasn't going to lift in time regardless. `LISTING_RETRY_DELAYS_SECONDS` is back to matching `RETRY_DELAYS_SECONDS` (`15, 30, 90`) — enough to ride out a genuine short blip, after which the listing fetch raises and the next scheduled run's normal catch-up picks the day up (no special handling needed: a listing-fetch failure happens before any announcement date is recorded complete, so `state_store`'s completed-date marker is untouched and the day stays pending). `RETRY_DELAYS_SECONDS` also covers PDF downloads (`generation_service.stage_pdf_download`). The listing and abstract-page fetches now send `arxiv_rate_limit.DEFAULT_HEADERS` (a realistic `Accept`/`Accept-Language`/`Accept-Encoding` set) instead of a bare User-Agent, since plain `urllib.request.Request` sends no `Accept` header and an `Accept-Encoding: identity` that real browsers essentially never send — both plausible bot signals independent of request volume, which fits the 406 recurring on the very first request of an otherwise-clean day far better than a rate/volume explanation does. On a transient status, `arxiv_rate_limit.describe_http_error` now logs the response's `Retry-After` header and body snippet (best-effort; arXiv's block page may explain the reason directly) instead of just the bare status code — if the header-realism change doesn't resolve the recurrence, that log output is the next diagnostic step, not another guess. The abstract-page fallback fetch (`arxiv_fetcher._fetch_abstract_html`) makes a single attempt with no retry of its own — it is itself a fallback path, reached only when the primary metadata query has already hit a transient error. Repeated rapid retries against an already-406'd resource still risk being read as an attack per arXiv's own robots.txt warning — so a stuck backlog of failed PDFs should be retried in a slow, spaced-out follow-up pass (or left for the next scheduled run), not forced through in one back-to-back sweep. There is no authenticated API/token for casual PDF access; arXiv's own guidance for a flagged legitimate use case is to contact the arXiv administrators in advance, not to request a key.
