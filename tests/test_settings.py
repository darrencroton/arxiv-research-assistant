...
def test_load_config_parses_explicit_llm_provider_settings(tmp_path: Path) -> None:
    ...
    assert config.min_summarize_score == 91.5
...
def test_load_config_uses_new_runtime_sections(tmp_path: Path) -> None:
    ...
    assert config.min_summarize_score == 90.0
...
def test_load_config_rejects_min_summarize_score_below_interest_threshold(tmp_path: Path) -> None:
    config_path = tmp_path / "settings.toml"
    config_path.write_text(
        f"{DEFAULT_HEADINGS}\n"
        "[arxiv]\n"
        "min_summarize_score = 65\n"
        "min_selection_score = 70\n",
        encoding="utf-8",
    )

    with pytest.raises(ValueError, match="min_summarize_score"):
        load_config(config_path)


def test_load_config_rejects_removed_always_summarize_score(tmp_path: Path) -> None:
    config_path = tmp_path / "settings.toml"
    config_path.write_text(
        f"{DEFAULT_HEADINGS}\n"
        "[arxiv]\n"
        "always_summarize_score = 90\n"
        "min_selection_score = 70\n",
        encoding="utf-8",
    )

    with pytest.raises(ValueError, match=r"\[arxiv\]\.always_summarize_score has been removed"):
        load_config(config_path)
